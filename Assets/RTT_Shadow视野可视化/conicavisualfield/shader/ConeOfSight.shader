Shader "GPUMan/ConeOfSight" 
{
	Properties{
		...
		//_CameraDepthTexture("CameraTex",2D)= "white" {}
		_NonVisibleColor("Non Visible Color",Color) = (0,0,0,1)
		_Colormy("Colormy",Color) = (1,1,1,1)
		...
		Subshader{
			...
			Pass {
				...
				float getRadiusAlpha(float distFromCenter)
				{
					...
					return innerCircleAlpha < 0.45 ? 0.5 : 0;
				}
				...
				float getAngleAlpha(float2 pos)
				{
				    ...
				    return max(1.0 - pow(angleF, 120), 0);
					//return angle < _ViewAngle ? 1 : 0;
				}
				...
				half4 frag(v2f i) : COLOR
				{
					...
					//half4 _Colormym = half4(0, 0, 1, 0);
					//half4 _Colormyn = half4(1, 1, 1, 1);
					half4 _Colormyy = distFromCenter > _myfen ? _Colormy : _Color;
					...
					float4 col = obstacleAlpha > 0 ? _Colormyy : _NonVisibleColor;
					col.a = obstacleAlpha > 0 ? 1 : 0;
					return saturate(float4(col.rgb, alpha * col.a ));
					//return float4(depth1, 0, 0, 1);
				}
				ENDCG
			}
		}
